class MastermindSolution
  ...
  attr_reader :solution_choices, :master_solution
  attr_accessor :solution

  def initialize params
    if params[:solution]
      @solution_choices = ('A'..'Z').to_a
      @master_solution = @solution = String.new(params[:solution]).split(//)
    else
      @solution_choices = params[:solution_choices].split(//)
      @master_solution = @solution = @solution_choices.shuffle.slice(0,params[:solution_length])
    end
  end
