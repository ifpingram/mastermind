...
describe Mastermind do
    ...
    [ # solution, attempt, success?, output
     ...
     ['RGBY','ACDE',false,'....'],
      ...
      it "outputs '#{solution_attempt_output[2]}' when the solution is '#{solution_attempt_output[0]}' and the guess is '#{solution_attempt_output[1]}'" do
        expect(Mastermind.new(Mastermind::Solution.new({:solution => solution_attempt_output[0]})).is_guess_correct?(solution_attempt_output[1])).to eq(solution_attempt_output[2])
      end
      it "outputs '#{solution_attempt_output[3]}' when the solution is '#{solution_attempt_output[0]}' and the guess is '#{solution_attempt_output[1]}'" do
        mastermind = Mastermind.new(Mastermind::Solution.new({:solution => solution_attempt_output[0]}))
  ...
  # is_guess_Correct?
  # mastermind.solution.solution eq('ABCD')

  context "testing input exception paths" do
    let(:mastermind) {Mastermind.new(Mastermind::Solution.new({:choices => 'RGBY', :length => 4}))}
  ...
  xcontext "making guesses: 1 slot, 1 color" do
    it "outputs correct color and position when the guess is correct" do
      expect(Mastermind.new(Mastermind::Solution.new({:choices => 'G', :length => 1})).check('G')).to eq([:match_color_and_position])
    end

    it "outputs no match when the guess is incorrect" do
      expect(Mastermind.new(Mastermind::Solution.new({:choices => 'G', :length => 1})).check('R')).to eq([:no_match])
    end
  end
  ...
  xcontext "making guesses: 2 slots, 4 colors" do
    it "outputs correct color and position when the guess is correct" do
      ...
      expect(Mastermind.new(Mastermind::Solution.new({:choices => 'RG', :length => 2})).check('RG')).to eq([:match_color_and_position, :match_color_and_position])
    end
    ...
    it "outputs partial matches when the guess is correct colours but wrong position" do
      allow_any_instance_of(Array).to receive(:shuffle) { ['R', 'G'] }
      expect(Mastermind.new(Mastermind::Solution.new({:choices => 'RG', :length => 2})).check('GR')).to eq([:match_color_not_position, :match_color_not_position])
    end
  ...
  xcontext "making guesses: 3 slots, 4 colors" do
    it "outputs [:match_color_not_position, :match_color_not_position, :match_color_not_position] when solution is 'RGR' and guess is 'GRG'" do
      ...
      expect(Mastermind.new(Mastermind::Solution.new({:choices => 'RGB', :length => 3})).check('BRG')).to eq([:match_color_not_position, :match_color_not_position, :match_color_not_position])
    end
  end
end