...
describe Mastermind do
  ...
  context "testing input exception paths" do
    let(:mastermind) {Mastermind.new(MastermindSolution.new({:choices => 'RGBY', :solution_length => 4}))}
  ...
  xcontext "making guesses: 1 slot, 1 color" do
    it "outputs correct color and position when the guess is correct" do
      expect(Mastermind.new(MastermindSolution.new({:choices => 'G', :solution_length => 1})).check('G')).to eq([:match_color_and_position])
    end

    it "outputs no match when the guess is incorrect" do
      expect(Mastermind.new(MastermindSolution.new({:choices => 'G', :solution_length => 1})).check('R')).to eq([:no_match])
    end
  end
  ...
  xcontext "making guesses: 2 slots, 4 colors" do
    it "outputs correct color and position when the guess is correct" do
      ...
      expect(Mastermind.new(MastermindSolution.new({:choices => 'RG', :solution_length => 2})).check('RG')).to eq([:match_color_and_position, :match_color_and_position])
    end

    it "outputs no match when the guess is incorrect" do
      allow_any_instance_of(Array).to receive(:shuffle) { ['R', 'G'] }
      expect(Mastermind.new(MastermindSolution.new({:choices => 'RG', :solution_length => 2})).check('BY')).to eq([:no_match, :no_match])
    end

    it "outputs partial matches when the guess is correct colours but wrong position" do
      allow_any_instance_of(Array).to receive(:shuffle) { ['R', 'G'] }
      expect(Mastermind.new(MastermindSolution.new({:choices => 'RG', :solution_length => 2})).check('GR')).to eq([:match_color_not_position, :match_color_not_position])
    end
  ...
  xcontext "making guesses: 3 slots, 4 colors" do
    it "outputs [:match_color_not_position, :match_color_not_position, :match_color_not_position] when solution is 'RGR' and guess is 'GRG'" do
      ...
      expect(Mastermind.new(MastermindSolution.new({:choices => 'RGB', :solution_length => 3})).check('BRG')).to eq([:match_color_not_position, :match_color_not_position, :match_color_not_position])
    end
  end
end